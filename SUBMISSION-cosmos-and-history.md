...
The **Cosmos and History** submission workflow lives on `main` as the script `make-cosmos-submission.sh`, which converts the canonical `main.tex` (LaTeX) into a Word document with footnote citations. No separate `main-cosmos.tex` is needed because the conversion happens at submission time via pandoc.
...
## Differences from the canonical pre-print

The canonical pre-print version (`main.tex`) is a 13,200-word LaTeX article with parenthetical natbib citations. The Cosmos and History submission produces a Word document with footnote citations from the same source.

| Element | Canonical pre-print | Cosmos and History submission |
|---|---|---|
...
## Related files on `main`

All on the same `main` branch (Overleaf single-branch constraint):

- `main.tex` — canonical pre-print version, what Zenodo points to
- `main-foundations.tex` — Foundations of Physics (Springer) submission variant, kept as backup. Compile by switching Overleaf's main-document setting to this file.
- `make-cosmos-submission.sh` — this submission's pandoc converter. Reads `main.tex`, writes `pub2-cosmos-and-history.docx`.
- `flatten-foundations.sh` — companion script for the Foundations version, in case you switch backup targets.
